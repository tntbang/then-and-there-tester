...
registerBackground({
  ...
  render(ctx, width, height, params, palette, rng) {
    ...
    // Use a tile size for performance at large resolutions
    const tileSize = Math.min(256, Math.min(width, height));
    ...
    for (let y = 0; y < tileSize; y++) {
      for (let x = 0; x < tileSize; x++) {
        ...
        noise += simplexNoise2D(x / 25, y / 25, seed + 1) * 0.25 * darkStreaks;
        noise += simplexNoise2D(x / 12, y / 12, seed + 2) * 0.15 * poreDetail;
        noise += simplexNoise2D(x / 6, y / 6, seed + 3) * 0.1 * poreDetail;
